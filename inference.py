...
def inference(input_image: Image, checkpoint_path: str='./checkpoint/CIHP_pgn') -> Image or None:
    # Create queue coordinator.
    coord = tf.train.Coordinator()
    # Load reader.
    with tf.name_scope("create_inputs"):
        tmpdir = tempfile.mkdtemp()
        tmpfile = os.path.join(tmpdir, 'temp.{}'.format(input_image.format))
        with open(tmpfile, 'w') as fp:
            input_image.save(fp, input_image.format)
        reader = ImageReader([tmpfile], None, False, False, False, coord)
        image = reader.image
    ...
    if checkpoint_path is not None:
        if load(loader, sess, checkpoint_path):
            print(" [*] Load SUCCESS")
        else:
            print(" [!] Load failed...")
            return None
